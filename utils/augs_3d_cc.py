...
def composite_blur_stack(blur_stack, dist_left, dist_right, values_left, values_right):
    ...
    if len(composite_vals.shape) == len(values_left.unsqueeze(1).shape):
        ...
        _ = composite_vals.scatter_(1, index=values_left.unsqueeze(0).unsqueeze(2), src=sim_left.unsqueeze(0).unsqueeze(2))
        _ = composite_vals.scatter_(1, index=values_right.unsqueeze(0).unsqueeze(2), src=sim_right.unsqueeze(0).unsqueeze(2))
...
def get_motion_stack_single_image(rgb, m_blur_radii):
    ...
    for ii in range(len(args)):
        ...
        ### Make sure m_blur_radii[ii].item() (kernal size) is odd and greater than 3 
        kernel_size = int(m_blur_radii[ii].item())
        if kernel_size <= 3 or kernel_size % 2 == 0:
            kernel_size = max(5, kernel_size | 1)  # Ensure it's an odd number and greater than 3
        
        module_curr = RandomMotionBlur(kernel_size, [angle_rand-0.001,angle_rand+0.001], 0.0, p=1.) # for kornia motion aug
